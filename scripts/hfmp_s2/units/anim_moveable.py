...
import pf
from hfmp_s2.constants import *
import weakref
from . import controllable as cont
import hfmp_s2.action
...
class AnimMoveable(pf.AnimEntity, pf.MovableEntity, cont.Controllable):
    ...
    def __init__(self, path, pfobj, name, **kwargs):
        pf.AnimEntity.__init__(self, path, pfobj, name, **kwargs)
        cont.Controllable.__init__(self, path, pfobj, name, **kwargs)
        self.moving = False
    ...
    def action(self, idx):
        if idx == 0:
            return hfmp_s2.action.ActionDesc(
