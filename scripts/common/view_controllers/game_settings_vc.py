...
from . import view_controller as vc
...
class GameSettingsVC(vc.ViewController):
    ...
    def __on_settings_apply(self, event):
        if self.view.hb_idx != self.__og_hb_idx:
            try:
                if self.view.hb_idx == 0:
                    pf.settings_set("pf.game.healthbar_mode", pf.HB_MODE_DAMAGED)
                else:
                    pf.settings_set("pf.game.healthbar_mode", pf.HB_MODE_NEVER)
                self.__og_hb_idx = self.view.hb_idx
            except Exception as e:
