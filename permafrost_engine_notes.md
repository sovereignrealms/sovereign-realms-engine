...
- The next native arm64 compile blockers are no longer renderer-loader-specific:
  ...
  - Python 3 now supports cooperative generator-style `pf.Task` scripts with `yield self.yield_()`, `yield self.sleep(ms)`, and `yield self.await_event(event)`; old stackful blocking task operations remain Python 2-only because they depend on removed CPython frame/thread-state internals
  - the only concrete helper in the live codebase still depending on old stackful `pf.Task` was `scripts/common/disappearing_text_task.py`; on Python 3 it now uses a module-level `EVENT_UPDATE_START` handler instead of task fibers, so the visible text-fade behavior no longer needs the old engine-level task internals
  - the Python 3 console now runs single-line and multiline input through the live console path, with a runtime self-test covering a compound `for` block
...
- The consolidated Metal-native flexible-world roadmap is saved at [plans/2026-04-24-metal-native-flexible-world-roadmap.md](/Users/dev/Desktop/OpenGL%20RTS%20game%20engine/plans/2026-04-24-metal-native-flexible-world-roadmap.md).
...
- The Metal-native fork work has now started from that stable baseline:
  ...
  - the default Apple Silicon repo path is now `METAL`; `RENDER_BACKEND=OPENGL` remains available as the explicit parity/reference path
...
- The first verified Metal result is now a little deeper than cold launch:
  ...
  - the next slice now also restores the first Metal skybox command path: the backend handles `R_GL_SkyboxLoad`, `R_GL_SkyboxFree`, and `R_GL_DrawSkybox`, loads the existing six-face cubemap into a Metal cube texture, draws it through a small MSL skybox pipeline, and uses the same cubemap as the scaled background in the offscreen water reflection scene; this is functional skybox/water-reflection parity first, not final visual matching
  - the next slice now also restores the first Metal light-state/material-lighting parity path: the backend handles `R_GL_SetAmbientLightColor`, `R_GL_SetLightEmitColor`, and `R_GL_SetLightPos`, passes that engine light state into the Metal terrain, mesh, and water-surface uniforms, and uses source material ambient/diffuse/specular values in the Metal mesh shader instead of only a temporary diffuse approximation
  - the next slice now also restores the first Metal terrain-conforming debug/vector and debug primitive command path: the backend handles `R_GL_DrawLine`, `R_GL_DrawQuad`, `R_GL_DrawFlowField`, `R_GL_DrawCombinedHRVO`, `R_GL_DrawOrigin`, `R_GL_DrawRay`, and `R_GL_DrawOBB`, builds sampled strips or line buffers against live terrain heights plus CPU-transformed colored line primitives, and directly verifies the chunk-boundary, last-command flow-field, and combat-target ray route with `scripts/macos/pf_metal_debug_overlay_probe.py`
  - the next slice now also restores the first Metal terrain patch-command/material-adjacency path: `R_GL_TilePatchVertsBlend`, `R_GL_TilePatchVertsSmooth`, and `R_GL_TileUpdate` are no longer dropped by the Metal dispatcher, Metal terrain receives flat Wang/blend adjacency attributes, and the latest fixed-camera capture shows cobblestone/road material transitions on Metal instead of the earlier raw grass-only terrain
  - the next slice now also restores the first Metal terrain sampler/mipmap parity path: Metal terrain arrays now allocate mip levels, generate those mipmaps with a blit encoder after upload, and bind a dedicated trilinear repeat sampler for terrain while keeping water DUDV/normal textures on the existing scene sampler
  - the next slice now also restores the first Metal terrain height-map normal/lighting parity path: Metal generates the same 2048x2048 octave Perlin height map as the OpenGL terrain path, binds it as a fragment buffer, passes `no_bump_map` through the terrain vertex output, and perturbs terrain normals with the same height-map weight before lighting
  - the next slice now also adds a water-with-units and rocks parity probe: the fixed-camera visual probe stages movable friendly units at a pathable water-edge target before capture, records that staging data in summary JSON, adds a dedicated rocks scene, moves the Metal water shader closer to the OpenGL DUDV/reflection/refraction formula, and adds a mipmapped trilinear material texture-array sampler for rocks/static props
	  - the next slice now also fixes the first visible Metal mesh material UV mismatch: Metal mesh material textures now sample the same UV orientation as the OpenGL `textured-phong-shadowed` path instead of applying an extra vertical flip, improving units and rocks/static props in the water/rocks parity capture
	  - the next slice now also fixes the first visible Metal static-prop winding/culling mismatch: Metal static mesh draws and dynamic static-vertex streams use clockwise front-facing winding to match OpenGL's `glFrontFace(GL_CW)`, while the later cliff-face shadow slice keeps the shadow depth pass counter-clockwise to match OpenGL's effective front-cull shadow caster behavior
	  - the next slice now also fixes the first Metal mesh normal-transform parity mismatch: mesh normal transforms are explicit in the Metal uniform block, CPU-skinned animated meshes use inverse-transpose pose/bind normal transforms like the OpenGL skinned shader, and the latest water/rocks/units capture verifies the path while still leaving broader color/material and water tone gaps
	  - the next slice now also fixes the first Metal unlit screen-space statusbar/healthbar tone mismatch: Metal draws screen-space colored triangles through the unlit world-color pipeline instead of the lit static-mesh material pipeline, so healthbars and simple status overlays keep authored red/green/black colors
	  - the next slice now also tightens Metal healthbar outline parity in the deterministic combat capture: the probe resets animated entities after pausing, Metal healthbars now match GL's effective 2 px border, outer-UV-space fill cutoff, and darker bottom-half fill, and the worst half=2 combat hot strip moved from 2.40,3.80,2.54 to 1.11,1.28,1.08 while the broader half=60 unit/cobblestone material-lighting residual remains open
	  - the next slice now also fixes the first Metal texture-array LOD-bias mismatch and tightens the parity harness: Metal terrain/material texture-array samples use the same `-0.5` mip LOD bias as OpenGL, and the visual probe now uses a deterministic free capture camera so OpenGL and Metal scene summaries report identical camera positions/directions
	  - the next slice now also matches the Metal mesh shadow shader formula to OpenGL's `textured-phong-shadowed` path: static/skinned meshes use the same nearest-depth comparison, `0.002` bias, and `0.55` shadow multiplier, leaving terrain shadow coverage to the follow-up parity slice
  - the next slice now also matches the first Metal terrain shadow coverage path to OpenGL's `terrain-shadowed.glsl` Poisson visibility heuristic: terrain shadows now use the same effective one-pass Poisson comparison, `0.002` bias, and `0.55` multiplier instead of the earlier Metal-only slope-biased 3x3 PCF path
  - the next slice now also restores the first Metal water/shore/fog parity foundation: depth-backed Metal water uses the same bilinear fog-of-war tint shape as OpenGL `water.glsl` and lets refraction depth shape the main-scene shoreline instead of hard-masking water to nearest tiles
  - the next slice now also restores the first Metal water material/timing parity fix: Metal advances the water DUDV move factor with the same `0.015` per-second wrapped clock as OpenGL and uses the same source-alpha blend factor for the water surface
  - the next slice now also restores the first Metal water sampled-scene parity fix: the reflection texture now uses the original scene camera for map/entity culling and the reflected camera only for the skybox, matching the OpenGL water reflection submission shape more closely
  - the next slice now also restores the first Metal offscreen water-scene color/tone fix: the offscreen reflection/refraction passes no longer run the terrain shader's temporary water overlay, so the final water surface samples terrain/entities/skybox rather than pre-tinted helper water; the latest paired capture still leaves shoreline/color response as an active visual-parity gap
  - the next slice now also restores the first Metal water final-color formula parity fix: the Metal water surface now follows OpenGL `water.glsl` more closely for normal-map channel interpretation, reflection UV clamp shape, full `float4` reflection/refraction mixing, tint/specular/alpha/fog order, and the latest matched-camera capture proves the remaining darker terrain/material and water shoreline/color differences are renderer gaps rather than camera-placement noise
	  ...
	  - the current MGL lesson fit for this project is to treat OpenGL as mutable state resolved at draw boundaries, then make each equivalent state explicit in Metal pipeline/encoder/render-pass setup; this reinforces the current parity approach around winding/culling, depth/blend state, render target load/store rules, shader bindings, sampler/mipmap behavior, and fixed-camera verification
	  - the current world-feedback slice is still intentionally narrow: unit/building selection shapes, drag-box UI, terrain-conforming debug/vector overlays and debug primitives, healthbars, a first Metal unlit screen-space statusbar/healthbar tone fix, a first minimap with terrain, unit dots, and clipped camera-frustum overlay, a first translucent map-overlay/faction-vision slice, a first scripted gameplay smoke pass, the richer Session UI roundtrip, live terrain fog-of-war shading, a first visible water slice plus richer DUDV/normal-driven terrain-path water, a depth-backed post-scene water-surface foundation, a first offscreen reflection/refraction texture stack, a first clipped/entity water-scene pass, a first refraction-depth-aware water-composition slice, a first water-with-units and rocks fixed-camera capture, a first Metal water/shore/fog parity foundation, a first Metal water material/timing parity fix, a first Metal water sampled-scene parity fix, a first Metal offscreen water-scene color/tone fix, a first Metal water final-color formula parity fix, a first Metal terrain/material texture-array LOD-bias fix, a first Metal material texture-array mipmap pass for rocks/static props, a first Metal mesh material UV parity fix for units and rocks/static props, a first Metal static-prop winding/culling parity fix, a first Metal mesh normal-transform parity fix, a first Metal mesh shadow formula parity fix, a first Metal static-mesh self-shadow parity fix for the cliff-face shadow-pass winding gap, a first Metal terrain shadow coverage parity fix, a first Metal skybox command path with skybox-fed water reflections, a first Metal light-state/material-lighting parity slice, a first terrain patch-command/material-adjacency pass, a first terrain sampler/mipmap parity pass, a first terrain height-map normal/lighting parity pass, a first real terrain-texturing pass, a first real textured static/skinned-mesh pass, a first real foliage/tree cutout-material path, a first Metal batch-rendering compatibility path, first true batch-performance paths for opaque static, opaque animated, and conservative translucent meshes, a first native Metal world-sprite batch path, real sprite effect fixtures/probe coverage for projectile trails, impacts, fire, and smoke, first gameplay-driven Mage projectile trail/impact coverage plus fire/smoke fixture coverage, a first UI command identity split, a first GL perf/query wrapper cleanup for Metal builds, a first Metal mesh-init/skybox loader-root split, a first Metal position/movement loader-root split, a first Metal texture-loader root split, a first on-screen MSAA pass, an explicit Metal MSAA parity state that defaults to OpenGL's no-MSAA baseline while keeping `PF_METAL_MSAA_SAMPLES=2/4` as an opt-in quality experiment, a first bounded in-flight frame-pacing slice, a main-frame depth-buffer foundation, first functional shadow-map parity, first shadow-filter/bias tuning, a deterministic fixed-camera visual capture harness, and the fixed-camera Metal magenta main-scene blocker fix all verify on Metal, but broader gameplay validation, remaining water shoreline/color response, broader character/static-prop color/material parity at water/rock camera distances, full water/skybox visual parity, richer terrain-material/color parity, and later parity features are still pending
  - `scripts/macos/pf_visual_parity_probe.py` and `scripts/macos/capture_visual_parity.sh` now provide the first fixed-camera OpenGL/Metal visual capture harness; the first artifact set is at `visual_parity_captures/2026-04-24-092629/`, the probe now uses a small CoreGraphics-backed Swift helper to resolve the actual macOS game-window id before `screencapture -l`, and the 2026-04-26 combat-determinism slice pauses simulation before scene construction plus fails paired captures if OpenGL/Metal camera positions drift
	  - the first parity capture is intentionally treated as evidence, not a pass: it exposed UI/minimap/healthbar/selection feedback over a magenta Metal main scene, the follow-up capture at `visual_parity_captures/2026-04-24-metal-msaa-store-water/` confirms that blocker is fixed by preserving the MSAA color target across the water-pass encoder end/resume path, the skybox/water-reflection capture is saved at `visual_parity_captures/2026-04-24-metal-skybox-water/`, the light-state capture is saved at `visual_parity_captures/2026-04-24-metal-light-state/`, the debug primitive capture is saved at `visual_parity_captures/2026-04-24-metal-debug-primitives/`, the terrain patch-command/material-adjacency capture is saved at `visual_parity_captures/2026-04-24-metal-terrain-patch-final-pair/`, the terrain sampler/mipmap capture is saved at `visual_parity_captures/2026-04-24-metal-terrain-mipmaps-windowid-final/`, the terrain height-map normal/lighting capture is saved at `visual_parity_captures/2026-04-24-metal-terrain-heightmap-normal/`, the water-with-units/rocks capture is saved at `visual_parity_captures/2026-04-24-metal-water-units-rocks/`, the mesh material UV parity capture is saved at `visual_parity_captures/2026-04-24-metal-mesh-uv-parity/`, the static-prop winding/culling parity capture is saved at `visual_parity_captures/2026-04-25-metal-static-prop-winding/`, the mesh normal-transform parity capture is saved at `visual_parity_captures/2026-04-25-metal-mesh-normal-parity/`, the unlit statusbar/healthbar tone capture is saved at `visual_parity_captures/2026-04-25-metal-unlit-statusbar-tone/`, the deterministic camera plus texture-array LOD-bias capture is saved at `visual_parity_captures/2026-04-25-metal-terrain-water-tone-lodbias/`, the mesh shadow formula parity capture is saved at `visual_parity_captures/2026-04-24-metal-mesh-shadow-parity/`, the terrain shadow coverage parity capture is saved at `visual_parity_captures/2026-04-24-metal-terrain-shadow-coverage/`, the water/shore/fog parity capture is saved at `visual_parity_captures/2026-04-24-metal-water-fog-parity/`, the water material/timing parity capture is saved at `visual_parity_captures/2026-04-24-metal-water-material-timing/`, the water sampled-scene parity capture is saved at `visual_parity_captures/2026-04-24-metal-water-scene-sampling/`, the offscreen water-scene color/tone capture is saved at `visual_parity_captures/2026-04-25-metal-water-offscreen-color/`, and the latest water final-color formula parity capture is saved at `visual_parity_captures/2026-04-25-metal-water-final-color-parity/`
  - the default Metal Apple Silicon binary no longer compiles `src/render/backend_gl.c` or links `OpenGL.framework`; the first swapchain-command cleanup also removes Metal references to `R_GL_SwapchainPresentLast` and `R_GL_SwapchainSetRes` from the shared loading-screen/resolution paths; frame, view/projection/light, screenspace/box, world/debug, selection/overlay, loading-screen/healthbar, skeleton/normals/model-preview, scene draw, mesh-init, depth-pass, map, water, skybox, minimap, tile, batch, animation, sprite, UI, optional position/movement GPU, and asset texture get/load commands are now shared backend-neutral command identities with OpenGL execution behind private `_Impl` or neutral `R_Tile*Impl` functions; Metal now also has a native world-sprite batch implementation plus real projectile-trail/impact/fire/smoke probe fixtures, gameplay-driven Mage projectile trail/impact coverage for that sprite command, direct native `R_Metal_UI_*` dispatch for UI commands, no current canonical Metal render-object references to OpenGL timer-query/debug-group wrappers, Metal-side asset dumps read CPU vertex mirrors instead of OpenGL VBOs, and dead-stripped macOS Metal links; the Metal runtime binary now has no live `_pf_gl*` loader symbols and no `OpenGL.framework` link
- The post-port graphics north star is a Metal-native HD/4K flexible-world platform:
  - Age of Empires II: Definitive Edition Enhanced Graphics Pack is a clarity/readability benchmark only, not a ruleset, art-style, compatibility, or asset target
  - useful quality lessons include Retina-crisp text/UI, high-DPI HUD/editor readability, crisp zoom-readable units/characters, wide zoom-out readability, dense battlefield composition, strong terrain/vegetation/building detail, careful arrows/fire/smoke/impact rendering, low input latency, and avoiding fuzzy visuals or unnatural over-smoothed motion
  - Retina readiness applies broadly: text, HUD/minimap/editor UI, characters, terrain, water, vegetation, buildings, projectiles, fire/smoke, and capture/output paths should avoid blurry post-rasterization scaling
  - the packaged Metal editor path now has full editing QA coverage: the self-contained `Permafrost Editor.app` can be opened through LaunchServices, audited for editor feature surfaces, used to edit terrain and place animated/static objects, saved to app-local map/scene files, reloaded fresh, and visually verified through nonblank Terrain/Objects screenshots
  - the historical feature audit is now covered by `scripts/macos/pf_macos_historical_features_probe.py`: Metal/OpenGL both verify PFMap serialization plus `load_map_string`, empty PFScene serialization plus `load_scene`, session export, representative static/animated PFOBJ structure, Blender exporter syntax, PerfStatsWindow/perf APIs, and Apple Silicon Wang-tileset fallback source markers; full Blender-driven export, the original Image Quilt generator, and native Metal GPU timestamp readback remain tooling/profiling follow-ups rather than gameplay-start blockers
  - first Retina/readability polish is now in place: Metal/OpenGL windows request high-DPI SDL drawables, SDL mouse events are scaled once into drawable coordinates before UI/game dispatch, and Nuklear default/TTF fonts use higher atlas oversampling
  - runtime Retina UI readability now has a Metal probe artifact: `scripts/macos/pf_metal_runtime_ui_readability_probe.py` captures HUD, Settings, Session, and Console UI at `3456x2234` from a `1728x1117` window with `highdpi=1`; Python error-dialog readability also has a controlled high-DPI screenshot
  - close/wide Retina scene readability now has a Metal probe artifact: `scripts/macos/pf_metal_retina_scene_readability_probe.py` captures close characters, close terrain/props, normal fogged wide gameplay, and a revealed wide-world overview at `3456x2234` from a `1728x1117` window with `retina_scale=[2.0, 2.0]`; the scene renderer is Retina-scale, while HD/4K character/world assets, richer terrain variation, denser vegetation/buildings, and zoom-aware LOD/readability remain post-port graphics-platform work
  - HD/4K world readability now has a target scaffold: `scripts/macos/pf_metal_hd_world_readability_probe.py` stages close heroes, dense armies, dense forest/building/prop content, projectile/fire/smoke/impact fixtures, and a wide large-map view, then captures five `3456x2234` Metal screenshots from a `1728x1117` window; latest artifact is `visual_parity_captures/2026-05-02-hd-world-readability-metal/`
  - the HD/4K scaffold proves the Metal renderer/capture path is ready for quality work, but the captures also show the next gap clearly: stock low-poly characters, repeated terrain texture patterns, limited biome/vegetation/building richness, and fixture-grade VFX need production assets plus zoom-aware LOD/readability rules
  - the interactive Python console is visible again on Python 3: the Nuklear console window renders, captures stdout/stderr, supports `flush()` without breaking probe logs, tees output to the original streams, and restores single-line execution; Python 3 multiline completeness parsing remains a future console polish item
	  - openage is an extensibility/modding architecture reference only: useful ideas include data-driven content, asset conversion/modpack thinking, configurable unit behavior, scripting hooks, and loose separation between simulation, renderer, input/presenter, events, world updates, and networking
	  - MGL is an OpenGL-on-Metal internals reference only: useful ideas include explicit GL-state-to-Metal-state mapping, draw-boundary validation, shader binding audits, and functional tests; it is not a target compatibility layer because this engine is being ported to native Metal
	  - the immediate renderer goal remains Metal parity, Metal default, and OpenGL removal; the HD/4K flexible-world graphics platform follows after the Metal-native port no longer depends on OpenGL
