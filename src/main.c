...
static enum render_status render_thread_poll(void);
static enum render_status render_thread_try_poll(void);
...
static enum render_status render_thread_wait_done_pumped(bool clear_events)
{
    enum render_status ret = render_thread_wait_done();

#if defined(__APPLE__) && defined(__aarch64__)
    while(ret == RSTAT_YIELD) {
        do {
            if(clear_events) {
                clear_sdl_events();
            } else {
            ...
            ret = render_thread_try_poll();
        } while(ret == RSTAT_NONE);
    }
#else
    (void)clear_events;
#endif
...
static enum render_status render_thread_wait_done_task(void)
{
...
static bool engine_init(void)
{
    ...
    Uint32 window_flags = R_WindowFlags();
    if(Settings_Get("pf.video.display_mode", &setting) == SS_OKAY) {
        wf = setting.as_int;
    }
    window_flags |= (Uint32)wf;
    ...
    render_thread_start_work();
    render_thread_wait_done_pumped(false);
    render_maybe_enable();
...
{
    R_WindowDrawableSize(s_window, out_w, out_h);
}
...
void Engine_FlushRenderWorkQueue(void)
{
    ...
    if(Sched_ActiveTID() == NULL_TID) {
        ...
        G_SwapBuffers();
        render_thread_start_work();
        status = render_thread_wait_done_pumped(false);
        assert(status == RSTAT_DONE);

    }else{
        assert(Sched_ActiveTID() != NULL_TID);
        /* When called from task context, the render thread is already busy
         * finishing the previously submitted frame. Wait for that work to
         * finish first, then swap in the newly queued commands and flush
         * those too.
         */
        status = render_thread_wait_done_task();
...
int main(int argc, char **argv)
{
    ...
    if(argc < 3) {
        printf("Usage: %s [base directory path (containing 'assets', 'shaders' and 'scripts' folders)] [script path] [script args...]\n", argv[0]);
        ret = EXIT_FAILURE;
        goto fail_args;
    }
    ...
    char **script_argv = script_argc ? argv + 3 : NULL;
...
#if defined(__APPLE__) && defined(__aarch64__)
    bool startup_ok = S_RunFile(argv[2], script_argc, script_argv);
    if(!startup_ok) {
        fprintf(stderr, "Startup script failed during synchronous native macOS bring-up.\n");
    }
    s_state = ENGINE_STATE_RUNNING;
#else
    S_RunFileAsync(argv[2], script_argc, script_argv, &s_request_done);
    ...
    while(!s_quit) {
        ...
        case ENGINE_STATE_RUNNING:
            ...
            render_status = render_thread_wait_done_pumped(false);
            G_SwapBuffers();
        ...
        case ENGINE_STATE_WAITING:
            ...
            clear_sdl_events();
            Sched_Tick();
            render_status = render_thread_wait_done_pumped(true);
