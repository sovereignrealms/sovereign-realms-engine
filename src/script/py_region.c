...
static PyObject *pyregion_wrap_existing(const char *name, enum region_type regtype);
static bool      pyregion_matches_existing(const char *name, enum region_type regtype,
                                           vec2_t position, float radius, vec2_t dims);
...
static PyMethodDef PyRegion_methods[] = {
    ...
    {"__pickle__", 
    (PyCFunction)PyRegion_pickle, METH_VARARGS | METH_KEYWORDS,
...
static PyObject *pyregion_wrap_existing(const char *name, enum region_type regtype)
{
    PyRegionObject *self = (PyRegionObject*)PyRegion_type.tp_alloc(&PyRegion_type, 0);
    if(!self)
        return NULL;
...
{
    vec2_t existing_pos;
    if(!G_Region_GetPos(name, &existing_pos))
        return false;

    if(fabsf(existing_pos.x - position.x) > 0.001f
    || fabsf(existing_pos.z - position.z) > 0.001f)
        return false;

    switch(regtype) {
    case REGION_CIRCLE: {
        float existing_radius = 0.0f;
        return G_Region_GetRadius(name, &existing_radius)
            && fabsf(existing_radius - radius) <= 0.001f;
    }
    case REGION_RECTANGLE: {
        float existing_x = 0.0f, existing_z = 0.0f;
        return G_Region_GetXLen(name, &existing_x)
            && G_Region_GetZLen(name, &existing_z)
            && fabsf(existing_x - dims.x) <= 0.001f
            && fabsf(existing_z - dims.z) <= 0.001f;
    }
    default:
        return false;
    }
                                     ...
                                     float radius, float xlen, float zlen)
{
    ...
    vec2_t dims = {xlen, zlen};
    if(pyregion_matches_existing(name, type, pos, radius, dims)) {
        PyObject *ret = pyregion_wrap_existing(name, type);
        if(ret) {
