...
static inline bool e_handlers_equal(struct handler_desc *a, struct handler_desc *b)
{
    if(a->type != b->type)
        return false;
...
static inline bool e_ptrs_equal(struct handler_desc *a, struct handler_desc *b)
{
    if(a->type != b->type)
        return false;
...
static uint64_t e_key(uint32_t ent_id, uint32_t event)
{
...
bool E_Global_Register(uint32_t event, handler_t handler, void *user_arg, int simmask)
{
...
bool E_Global_Unregister(uint32_t event, handler_t handler)
{
    struct handler_desc hd;
    hd.type = HANDLER_TYPE_ENGINE;
    hd.handler.as_function = handler;
