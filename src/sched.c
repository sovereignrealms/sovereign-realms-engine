...
void        sched_task_entry_trampoline(void);
...
static uint32_t         s_deferred_free_tid = NULL_TID;
...
static void sched_init_ctx(struct task *task, void *code)
{
    ...
    task->ctx.x19 = (uintptr_t)code;
    task->ctx.lr = (uintptr_t)sched_task_exit_trampoline;
    task->ctx.sp = (uintptr_t)stack_base;
    task->ctx.pc = (uintptr_t)sched_task_entry_trampoline;
    task->ctx.fpcr = 0;
    task->ctx.fpsr = 0;
#else
...
#ifndef _MSC_VER
#if defined(__aarch64__) && defined(__APPLE__)
    retaddr = (uint64_t)__builtin_return_address(0);
#else
    uint64_t rbp;
...
static bool stack_pointer_valid(const struct task *task)
{
    ...
    uintptr_t sp = sched_task_get_sp(task);
#if defined(__aarch64__) && defined(__APPLE__)
    if(((uint64_t)sp) & 0xf)
...
static bool sched_should_defer_shutdown(const struct task *task)
{
#if defined(__APPLE__) && defined(__aarch64__)
    return !!(task->name && 0 == strcmp(task->name, "session_task"));
#else
    (void)task;
    return false;
#endif
...
static void sched_task_run(struct task *task)
{
    ...
    PERF_POP();
    bool sp_valid = stack_pointer_valid(task);
#if defined(__APPLE__) && defined(__aarch64__)
    if(!sp_valid && task->name && 0 == strcmp(task->name, "session_task")) {
        fprintf(stderr, "[sched] ignoring invalid post-return stack pointer for session_task on Apple arm64\n");
    }else{
        assert(sp_valid);
    }
#else
...
static void sched_drain_deferred_free(void)
{
    if(s_deferred_free_tid == NULL_TID)
        return;

    struct task *task = &s_tasks[s_deferred_free_tid - 1];
    s_deferred_free_tid = NULL_TID;
...
static void sched_task_service_request(struct task *task)
{
    ...
    case _SCHED_REQ_FREE:
        if(sched_should_defer_shutdown(task)) {
            assert(s_deferred_free_tid == NULL_TID || s_deferred_free_tid == task->tid);
            task->state = TASK_STATE_ZOMBIE;
            s_deferred_free_tid = task->tid;
        }else{
            sched_task_shutdown(task);
        }
        break;
...
static bool work_exists_locked(void)
{
    return pq_size(&s_ready_queue_main) || pq_size(&s_ready_queue);
}
...
void Sched_Tick(void)
{
    ...
    do{
        ...
        SDL_LockMutex(s_ready_lock);
        while(!work_exists_locked()
           && ((nwaiters = s_nwaiters) < s_nworkers)
           && ((idle_workers = s_idle_workers) < s_nworkers)
           && !s_flushing) {
        ...
        if(curr == NULL && nwaiters == s_nworkers)
            break;
        if(curr == NULL && idle_workers == s_nworkers)
            break;
        if(curr == NULL)
            continue;
