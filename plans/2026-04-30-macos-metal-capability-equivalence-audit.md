...
| Editor | `EDITOR_FEATURE_AUDIT_READY backend=METAL renderer=Apple M2 Max factions=2`; `EDITOR_WORKFLOW_READY backend=METAL ... placed_objects=2 saved_objects=2`; `EDITOR_WORKFLOW_RELOAD_READY backend=METAL ... loaded_objects=2`; `EDITOR_VISUAL_READY backend=METAL renderer=Apple M2 Max captures=2 placed_objects=2 saved_objects=2`; self-contained `dist/Permafrost Editor.app` packaging verifies through macOS `open` and Computer Use now sees the rendered `Permafrost Editor — org.permafrostengine.editor.dev` window; OpenGL sanity also passed |
...
| Map/scene editor | Smoke verified | Metal editor launch, feature audit, terrain/object save, fresh saved map/scene reload, and deterministic screenshot verification pass. The visual harness paints a cobblestone patch, places animated/static objects, captures Terrain and Objects tabs with window-specific screenshots and a full-screen fallback, validates the PNGs as nonblank, and saves/reloads the edited map/scene. The self-contained macOS app bundle now gives the editor a normal `Permafrost Editor` app identity and rendered window for Computer Use attach checks; deeper manual editing flows remain a follow-up. |
