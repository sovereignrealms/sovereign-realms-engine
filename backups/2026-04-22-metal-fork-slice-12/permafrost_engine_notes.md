...
- The first verified Metal result is now a little deeper than cold launch:
  ...
  - the current world-feedback slice is still intentionally narrow: unit/building selection shapes, drag-box UI, and a first minimap with terrain, unit dots, and clipped camera-frustum overlay now render on Metal, but fog/water integration, gameplay validation, and later parity features are still pending
