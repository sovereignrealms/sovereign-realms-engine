...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches startup UI, visible terrain, first static meshes, first skinned-mesh rendering, first world-selection overlays, and drag-box UI selection on Apple Silicon`
...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the current Metal runtime is still intentionally limited to first-pass world feedback; minimap, gameplay parity, and batch parity are still pending
...
- `completed` The first Metal drag-box UI slice now also verifies on Apple Silicon:
  ...
  - the stable OpenGL fallback was rebuilt and rechecked immediately afterward with `./bin/pf-arm64 ./ ./scripts/macos/pf_native_launch_probe.py`, which again reported `pf.render.backend=OPENGL`
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, first world-selection overlays, and drag-box UI selection; minimap, normal play, and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - minimap and normal gameplay flow are still pending
