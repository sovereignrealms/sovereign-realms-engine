...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the minimap now has a first Metal terrain-only slice: the backend bakes a simple top-down terrain texture and renders the rotated minimap diamond on-screen
  - the current Metal runtime is still intentionally limited to first-pass world feedback; minimap unit dots, frustum, fog/water integration, gameplay parity, and batch parity are still pending
...
- `completed` The first Metal minimap slice now also verifies on Apple Silicon:
  ...
  - the current slice is intentionally narrow: it bakes a terrain-only top-down minimap texture and renders the rotated minimap diamond on-screen
  - the clean live screenshot for this slice is saved at `backups/2026-04-22-metal-fork-slice-9/pf_metal_minimap_live.png`
  - minimap unit dots, camera-frustum overlay, fog/water integration, and broader gameplay validation remain intentionally deferred for later Metal slices
  - the stable OpenGL fallback was rebuilt and rechecked immediately afterward with `./bin/pf-arm64 ./ ./scripts/macos/pf_native_launch_probe.py`, which again reported `pf.render.backend=OPENGL`
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, and a first terrain-only minimap render; normal play and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - a first terrain-only minimap render now appears on Metal
  - minimap unit dots, frustum, fog/water integration, and normal gameplay flow are still pending
