...
- The first verified Metal result is now a little deeper than cold launch:
  ...
  - the next slice now also routes the first minimap path through the Metal backend: it bakes a terrain-only top-down minimap texture and renders the rotated minimap diamond on-screen; the current screenshot is saved at `backups/2026-04-22-metal-fork-slice-9/pf_metal_minimap_live.png`
  - because the engine still routes visible entities through `R_GL_Batch_Draw` when batching is enabled, the Metal path currently forces `pf.video.use_batch_rendering` off so static meshes can advance through the existing per-entity `R_GL_Draw` path before Metal batch parity is implemented
  - the current world-feedback slice is still intentionally narrow: unit/building selection shapes, drag-box UI, and a first terrain-only minimap now render on Metal, but minimap unit dots, frustum, fog/water integration, gameplay validation, and later parity features are still pending
