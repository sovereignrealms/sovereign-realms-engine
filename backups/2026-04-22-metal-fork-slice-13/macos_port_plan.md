...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches startup UI, visible terrain, first static meshes, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, first healthbars, and a first minimap render with terrain, unit dots, and camera-frustum overlay on Apple Silicon`
...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the current Metal runtime is still intentionally limited to first-pass world feedback; fog/water integration, gameplay parity, and batch parity are still pending
