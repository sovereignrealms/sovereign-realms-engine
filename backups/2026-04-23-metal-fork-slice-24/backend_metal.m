...
static void render_batched_stat_entities(const vec_rstat_t *ents)
{
    ...
    if(!consumed) {
        for(int i = 0; i < nents; i++) {
            ...
            if(!priv || !priv->metal_is_static_mesh || priv->uses_pose_buffer)
                continue;
            render_static_mesh_draw(priv, &curr->model);
        }
        return;
    }
    ...
    for(int i = 0; i < nents; i++) {
        ...
        if(curr->translucent || !priv->metal_static_verts || !priv->mesh.num_verts) {
            consumed[i] = true;
            render_static_mesh_draw(priv, &curr->model);
            continue;
        }
        ...
        if(!combined) {
            ...
            for(int j = i + 1; j < nents; j++) {
                ...
                if(other->translucent || other->render_private != priv)
                    continue;
                render_static_mesh_draw(priv, &other->model);
            }
            continue;
        }
        ...
        mat4x4_t identity;
        PFM_Mat4x4_Identity(&identity);
        render_static_vertex_stream(priv, &identity, combined, dst_idx * sizeof(*combined));
        free(combined);
    }
...
static void render_batched_anim_entities(const vec_ranim_t *ents)
{
    ...
    if(!consumed) {
        for(int i = 0; i < nents; i++) {
            ...
            render_skinned_mesh_draw(priv, &curr->model);
        }
        return;
    }
    ...
    for(int i = 0; i < nents; i++) {
        ...
        if(curr->translucent || !priv->metal_anim_verts || !priv->mesh.num_verts) {
            consumed[i] = true;
            s_curr_anim_uid = curr->uid;
            s_have_anim_uid = true;
            render_skinned_mesh_draw(priv, &curr->model);
        ...
        if(!combined) {
            ...
            for(int j = i + 1; j < nents; j++) {
                ...
                s_have_anim_uid = true;
                render_skinned_mesh_draw(priv, &other->model);
            }
            continue;
        }
