...
- The first verified Metal result is now a little deeper than cold launch:
  ...
  - the next slice now also adds the first Metal refraction-depth-aware water-composition pass: the water-surface shader samples the offscreen refraction depth texture, linearizes scene and water depth like the OpenGL path, uses that difference to drive water alpha, and applies the same first shoreline boundary-discard heuristic to reduce hard refraction seams around water edges
