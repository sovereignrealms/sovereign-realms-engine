...
"    return 1.0;\n"
"}\n"
"fragment float4 terrain_fragment(TerrainVertexOut in [[stage_in]], constant TerrainUniforms &uniforms [[buffer(1)]], device const uchar *fog_buff [[buffer(2)]], device const uchar *water_buff [[buffer(3)]], texture2d_array<float> terrain_textures [[texture(0)]], sampler terrain_sampler [[sampler(0)]]) {\n"
...
"                     + uint(tile_c);\n"
"        if(uniforms.effect_params.z > 0.5) {\n"
"            uint fog_state = fog_buff[fog_idx];\n"
"            fog_factor = terrain_fog_factor(fog_state);\n"
"        }\n"
"        if(uniforms.effect_params.y > 0.5) {\n"
"            water_state = water_buff[fog_idx];\n"
"        }\n"
"    }\n"
"    if(uniforms.effect_params.w > 0.5) {\n"
"        uint texture_count = uint(uniforms.effect_params.w + 0.5);\n"
...
"    if(water_state != 0u) {\n"
"        float wave = 0.5 + 0.5 * sin((in.world_xz.x * 0.055) + uniforms.effect_params.x * 4.0 + (in.world_xz.y * 0.040));\n"
"        float ripple = 0.5 + 0.5 * sin((in.world_xz.x * 0.035) - uniforms.effect_params.x * 2.5 + (in.world_xz.y * 0.060));\n"
"        float3 deep = float3(0.04, 0.19, 0.34);\n"
"        float3 shallow = float3(0.08, 0.37, 0.56);\n"
"        float3 highlight = float3(0.32, 0.56, 0.74);\n"
"        lit = mix(deep, shallow, wave);\n"
"        lit = mix(lit, highlight, 0.20 + ripple * 0.18);\n"
"    }\n"
...
"    return out;\n"
"}\n"
"fragment float4 static_mesh_fragment(StaticMeshVertexOut in [[stage_in]], constant StaticMeshUniforms &uniforms [[buffer(1)]], texture2d_array<float> material_textures [[texture(0)]], sampler material_sampler [[sampler(0)]]) {\n"
"    float3 light_dir = normalize(float3(0.35, 0.85, 0.20));\n"
"    float diffuse = max(dot(normalize(in.normal), light_dir), 0.18);\n"
...
"    return float4(base * diffuse + ambient, base_rgba.w);\n"
"}\n";
...
static id<MTLRenderPipelineState> build_static_mesh_pipeline(bool translucent)
{
    NSError *error = nil;
...
static bool ensure_static_mesh_pipeline(void)
{
    if(s_static_mesh_pipeline)
        return true;
    s_static_mesh_pipeline = build_static_mesh_pipeline(false);
    return s_static_mesh_pipeline != nil;
}

static bool ensure_static_mesh_blend_pipeline(void)
{
    if(s_static_mesh_blend_pipeline)
        return true;
    s_static_mesh_blend_pipeline = build_static_mesh_pipeline(true);
    return s_static_mesh_blend_pipeline != nil;
}

static bool ensure_world_color_pipeline(void)
{
    if(s_world_color_pipeline)
        return true;
    ...
    s_world_color_pipeline = [s_device newRenderPipelineStateWithDescriptor:pipeline_desc error:&error];
    if(!s_world_color_pipeline) {
                                        ...
                                        bool translucent)
{
    ...
    if(!s_have_scene_view || !s_have_scene_proj)
        return;
    if(!(translucent ? ensure_static_mesh_blend_pipeline() : ensure_static_mesh_pipeline()))
        return;
    ...
    [s_frame_encoder setRenderPipelineState:translucent ? s_static_mesh_blend_pipeline : s_static_mesh_pipeline];
    [s_frame_encoder setCullMode:mutable_priv->metal_materials_have_cutout_alpha ? MTLCullModeNone : MTLCullModeBack];
    ...
    if(texture_array && ensure_scene_sampler()) {
        [s_frame_encoder setFragmentTexture:texture_array atIndex:0];
                                       ...
                                       MTLPrimitiveType primitive)
{
    ...
    if(!s_have_scene_view || !s_have_scene_proj)
        return;
    if(!ensure_world_color_pipeline())
        return;
    ...
    [s_frame_encoder setRenderPipelineState:s_world_color_pipeline];
    [s_frame_encoder setCullMode:MTLCullModeNone];
    [s_frame_encoder setFrontFacingWinding:MTLWindingCounterClockwise];
    [s_frame_encoder setVertexBuffer:vertex_buffer offset:0 atIndex:0];
    [s_frame_encoder setVertexBuffer:uniform_buffer offset:0 atIndex:1];
...
static void dispatch_or_drop_cmd(struct rcmd cmd)
{
    ...
    || cmd.func == R_GL_Batch_RenderDepthMap) {
        return;
    }
