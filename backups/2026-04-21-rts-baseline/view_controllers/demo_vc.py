...
class DemoVC(vc.ViewController):
    ...
    def __flash_session_status(self, text, color):
        dtt.DisappearingTextTask(
            text,
            DemoVC.STATUS_BOUNDS,
            color,
            DemoVC.STATUS_DURATION_MS
        ).run()
    ...
    def __on_session_saved(self, event):
        del event
        self.__flash_session_status("Session saved", (140, 255, 140, 255))

    def __on_session_loaded(self, event):
        del event
        self.__flash_session_status("Session loaded", (140, 255, 140, 255))

    def __on_session_fail_save(self, event):
        self.__flash_session_status("Save failed", (255, 120, 120, 255))

    def __on_session_fail_load(self, event):
        self.__flash_session_status("Load failed", (255, 120, 120, 255))
    ...
    def activate(self):
        ...
        pf.register_ui_event_handler(pf.EVENT_SESSION_SAVED, DemoVC.__on_session_saved, self)
        pf.register_ui_event_handler(pf.EVENT_SESSION_LOADED, DemoVC.__on_session_loaded, self)
        pf.register_ui_event_handler(pf.EVENT_SESSION_FAIL_SAVE, DemoVC.__on_session_fail_save, self)
        pf.register_ui_event_handler(pf.EVENT_SESSION_FAIL_LOAD, DemoVC.__on_session_fail_load, self)
        self.__view.show()

    def deactivate(self):
        self.__view.hide()
        pf.unregister_event_handler(pf.EVENT_SESSION_FAIL_LOAD, DemoVC.__on_session_fail_load)
        pf.unregister_event_handler(pf.EVENT_SESSION_FAIL_SAVE, DemoVC.__on_session_fail_save)
        pf.unregister_event_handler(pf.EVENT_SESSION_LOADED, DemoVC.__on_session_loaded)
        pf.unregister_event_handler(pf.EVENT_SESSION_SAVED, DemoVC.__on_session_saved)
