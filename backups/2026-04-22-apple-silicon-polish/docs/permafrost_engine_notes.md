...
- Running the stock game/demo natively on this Apple Silicon Mac is now feasible enough for active bring-up, visible-demo validation, and a first basic interaction smoke test.
- The project still needed non-trivial porting work to reach this point, but the current RTS smoke-coverage milestone is now treated as complete for this bring-up tracker.
- The most realistic near-term options are now:
  - Continue the native macOS/arm64 validation path from the now-completed RTS smoke-coverage milestone into the next renderer/runtime polish tasks.
  - Treat Permafrost as a reference architecture and plan a selective reuse strategy while the native bring-up matures.
  - Use a Linux or Windows environment if you want full stock-engine behavior sooner than the remaining native polish work.
