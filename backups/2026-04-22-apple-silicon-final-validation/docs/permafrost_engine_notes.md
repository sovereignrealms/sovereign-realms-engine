...
- The most realistic near-term options are now:
  - Continue the native macOS/arm64 validation path into a final game-polish pass: longer gameplay, settings persistence, and relaunch confidence.
  - Treat Permafrost as a reference architecture and plan a selective reuse strategy while the native Mac baseline stays stable.
  - Use a Linux or Windows environment only if you need full editor or Rosetta-era parity sooner than the current deferred backlog allows.
