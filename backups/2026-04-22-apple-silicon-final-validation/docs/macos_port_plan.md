...
- Current milestone status: `in progress; the native arm64 game path is now working, Mac-tuned defaults are being locked into the repo, and the remaining work is game-quality polish rather than first bring-up`
...
- `Phase 3: Apple Silicon game polish` - `in progress`
...
- `Now`
  - lock the Apple Silicon shipping baseline in repo defaults and tracked config
  - keep the existing game stable through launch, relaunch, settings, and Session save/load
  - add repeatable native validation helpers under `scripts/macos`
  - finish broader manual gameplay/settings validation on the real game UI
...
- `partial` Complete the broader Apple Silicon manual gameplay/settings validation pass.
  - The automated launch/relaunch, Session UI, and settings persistence probes are now green.
  - A longer user-driven 10-15 minute gameplay/settings pass still remains as the main current-scope manual item.
