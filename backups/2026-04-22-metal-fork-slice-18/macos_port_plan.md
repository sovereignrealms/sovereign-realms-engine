...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches startup UI, visible terrain, first static meshes, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, first healthbars, a first minimap render with terrain, unit dots, and camera-frustum overlay, a first translucent map-overlay-quads slice, a first scripted gameplay smoke pass, and a first Session UI save/load roundtrip on Apple Silicon`
...
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, first healthbars, a first minimap render with terrain, unit dots, and clipped camera-frustum overlay, translucent map-overlay quads, a first scripted gameplay smoke pass, the richer Python 3 Session UI save/load roundtrip, live terrain fog-of-war shading, and a first visible water slice; broader normal play, full reflection/refraction water parity, full material/texturing fidelity, and editor support are still pending migration work.
