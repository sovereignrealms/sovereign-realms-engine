...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches startup UI, visible terrain, first static meshes, and first skinned-mesh rendering on Apple Silicon`
...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the current animated-unit slice is still intentionally limited to first-pass silhouette/material-diffuse rendering; minimap, selection overlays, gameplay parity, and batch parity are still pending
...
- `completed` The first Metal skinned-mesh slice now also verifies on Apple Silicon:
  ...
  - the stable OpenGL fallback was rebuilt and rechecked immediately afterward with `./bin/pf-arm64 ./ ./scripts/macos/pf_native_launch_probe.py`, which again reported `pf.render.backend=OPENGL`
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, and first skinned-mesh rendering; minimap, selection overlays, normal play, and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - minimap, selection overlays, and normal gameplay flow are still pending
