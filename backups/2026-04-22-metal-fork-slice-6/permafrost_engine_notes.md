...
- The first verified Metal result is now a little deeper than cold launch:
  ...
  - the current animated-unit slice is still first-pass only: units now render on Metal, but minimap, selection overlays, gameplay validation, and later parity features are still pending
