...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the current Metal runtime is still intentionally limited to first-pass world feedback; broader fog/gameplay parity, water integration, and batch parity are still pending
...
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, first healthbars, and a first minimap render with terrain, unit dots, and clipped camera-frustum overlay; normal play and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - fog/water integration and normal gameplay flow are still pending
