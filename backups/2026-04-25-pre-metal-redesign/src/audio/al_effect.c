...
static bool effects_equal(struct al_effect *a, struct al_effect *b)
{
    return (a->uid == b->uid);
}

static bool effects_equal_qt(const struct al_effect *a, const struct al_effect *b)
{
    return (a->uid == b->uid);
}
...
static void on_new_map(void *user, void *event)
{
    ...
    qt_effect_init(&s_effect_tree, xmin, xmax, zmin, zmax, effects_equal_qt);
...
bool Audio_Effect_Init(void)
{
    ...
    qt_effect_init(&s_effect_tree, min, max, min, max, effects_equal_qt);
