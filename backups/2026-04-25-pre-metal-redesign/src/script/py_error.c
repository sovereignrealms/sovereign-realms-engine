...
static const char *s_traceback_filename(PyTracebackObject *tb, PyObject **out_filename)
{
    *out_filename = NULL;
    ...
    *out_filename = filename;
    return PyString_AsString(filename);
#else
    *out_filename = tb->tb_frame->f_code->co_filename;
    Py_INCREF(*out_filename);
    return PyString_AsString(*out_filename);
#endif
}
        ...
        if(err_ctx->traceback) {
            ...
            while (tb != NULL) {
                if (depth <= 128) {
                    ...
                    const char *filename = s_traceback_filename(tb, &filename_obj);
                    if(!filename) {
                        PyErr_Clear();
                        filename = "<unknown>";
                    }
                    int lineno = PyFrame_GetLineNumber(tb->tb_frame);
                    ...
                    char linebuff[512] = "";
                    s_print_source_line(filename, lineno, 4,
