...
static id<MTLRenderPipelineState> build_terrain_pipeline(NSUInteger sample_count)
{
    NSError *error = nil;
...
static id<MTLRenderPipelineState> ensure_terrain_pipeline(bool multisampled)
{
    id<MTLRenderPipelineState> __strong *slot = multisampled ? &s_terrain_msaa_pipeline : &s_terrain_pipeline;
    if(*slot)
        return *slot;
    *slot = build_terrain_pipeline(multisampled ? METAL_MSAA_SAMPLES : 1);
    return *slot;
}
...
static id<MTLRenderPipelineState> ensure_static_mesh_pipeline(bool translucent, bool multisampled)
{
    ...
    if(*slot)
        return *slot;
    *slot = build_static_mesh_pipeline(translucent, multisampled ? METAL_MSAA_SAMPLES : 1);
    return *slot;
}
                                    ...
                                    matrix_float4x4 proj)
{
    ...
    if(!priv->metal_terrain_verts || !priv->metal_terrain_verts_size || !priv->mesh.num_verts)
        return;
    pipeline = ensure_terrain_pipeline(multisampled);
    if(!pipeline)
        return;
                                        ...
                                        bool translucent)
{
    ...
    if(!s_frame_encoder)
        return;
    pipeline = ensure_static_mesh_pipeline(translucent, frame_uses_msaa());
    if(!pipeline)
        return;
...
static void render_world_colored_strip(const vec3_t *positions, size_t nverts, const vec3_t *color)
{
    ...
    pipeline = ensure_static_mesh_pipeline(false, frame_uses_msaa());
    if(!pipeline) {
        free(verts);
        return;
    }
...
static void render_minimap_bake(const struct map *map, void **chunk_rprivates, mat4x4_t *chunk_model_mats)
{
    if(!map || !chunk_rprivates || !chunk_model_mats)
        return;
    if(!ensure_terrain_pipeline(false))
        return;
