...
- The Metal-native fork work has now started from that stable baseline:
  ...
  - backend switches force a rebuild through a backend stamp so `obj/` artifacts are no longer silently reused across renderer modes
...
- The first verified Metal result is intentionally narrow:
  - `make pf PLAT=MACOS_ARM64 MACOS_ARM64_BUILD_READY=1 RENDER_BACKEND=METAL` succeeds
  - `./bin/pf-arm64 ./ ./scripts/macos/pf_metal_launch_probe.py` now survives engine startup and reports `pf.render.backend=METAL`
  - render commands are still dropped on the Metal path today, so this is a cold-launch bootstrap milestone rather than a playable-scene milestone
...
- The most realistic near-term options are now:
  - Keep the Apple Silicon OpenGL baseline stable as the fallback while the Metal backend advances through cold launch, playable runtime scene, then editor support.
