...
- Current milestone status: `in progress; the first backend-abstraction slice is now landed, with explicit OPENGL/METAL backend selection, a preserved OpenGL fallback, and a verified Metal cold-launch probe on Apple Silicon`
...
- The renderer control surface now has an explicit backend selector for Apple Silicon work:
  ...
  - backend changes now force object recompilation through a backend stamp instead of silently reusing stale `obj/` artifacts
...
- The first Metal bootstrap slice is intentionally narrow:
  - Metal now creates an SDL Metal view, a Metal device, command queue, and CAMetalLayer-backed clear/present loop
  - render commands are still dropped on the Metal path while the draw-call migration is pending
...
- `completed` The first Metal backend split now builds and verifies on Apple Silicon:
  ...
  - the backend stamp now forces recompilation when switching between `OPENGL` and `METAL`
- `partial` The Metal backend currently verifies cold launch only; runtime scene rendering, normal play, and editor support are still pending migration work.
...
- `pending` Port the runtime playable scene to Metal (terrain, meshes, UI, minimap, scene presentation).
