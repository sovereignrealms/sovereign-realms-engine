...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the current Metal runtime is still intentionally limited to first-pass world feedback plus that first scripted gameplay flow; broader fog/gameplay parity, water integration, Session validation on Metal, and batch parity are still pending
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - fog/water integration, Session roundtrip validation on Metal, and broader normal gameplay flow are still pending
