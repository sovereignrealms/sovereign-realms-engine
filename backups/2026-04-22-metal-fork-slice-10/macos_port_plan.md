...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - the minimap now has a first Metal slice: the backend bakes a simple top-down terrain texture, renders the rotated minimap diamond on-screen, and overlays unit dots through the same normalized minimap-space math as the OpenGL path
  - the current Metal runtime is still intentionally limited to first-pass world feedback; minimap frustum, fog/water integration, gameplay parity, and batch parity are still pending
...
- `completed` The first Metal minimap slice now also verifies on Apple Silicon:
  ...
  - the current slice is intentionally narrow: it bakes a top-down terrain texture, renders the rotated minimap diamond on-screen, and now also routes `R_GL_MinimapRenderUnits` through Metal using the same normalized minimap-space dot sizing and transform as the OpenGL path
  - the clean live screenshot for this slice is saved at `backups/2026-04-22-metal-fork-slice-10/pf_metal_minimap_units_live.png`
  - minimap camera-frustum overlay, fog/water integration, and broader gameplay validation remain intentionally deferred for later Metal slices
  - the stable OpenGL fallback was rebuilt and rechecked immediately afterward with `./bin/pf-arm64 ./ ./scripts/macos/pf_native_launch_probe.py`, which again reported `pf.render.backend=OPENGL`
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, first world-selection overlays, drag-box UI selection, and a first minimap render with terrain plus unit dots; normal play and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - a first minimap render with terrain plus unit dots now appears on Metal
  - minimap frustum, fog/water integration, and normal gameplay flow are still pending
