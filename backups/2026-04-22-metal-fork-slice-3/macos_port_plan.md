...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches a visible startup UI/menu milestone on Apple Silicon`
...
- `partial` The Metal backend now verifies cold launch plus visible startup UI/menu; runtime world rendering, normal play, and editor support are still pending migration work.
...
- `pending` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
