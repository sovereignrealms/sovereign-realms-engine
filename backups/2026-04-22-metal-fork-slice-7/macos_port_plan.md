...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches startup UI, visible terrain, first static meshes, first skinned-mesh rendering, and first world-selection overlays on Apple Silicon`
...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - drag-box UI selection and minimap work are still intentionally deferred behind this smaller world-feedback slice
  - the current Metal runtime is still intentionally limited to first-pass world feedback; minimap, drag-box UI, gameplay parity, and batch parity are still pending
...
- `completed` The first Metal world-selection-overlay slice now also verifies on Apple Silicon:
  ...
  - drag-box UI selection (`R_GL_DrawBox2D`) and the minimap subsystem remain intentionally deferred for later Metal slices
  - the stable OpenGL fallback was rebuilt and rechecked immediately afterward with `./bin/pf-arm64 ./ ./scripts/macos/pf_native_launch_probe.py`, which again reported `pf.render.backend=OPENGL`
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, visible terrain, first static-mesh rendering, first skinned-mesh rendering, and first world-selection overlays; drag-box UI, minimap, normal play, and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - drag-box UI, minimap, and normal gameplay flow are still pending
