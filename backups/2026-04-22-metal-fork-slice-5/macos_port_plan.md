...
- Current milestone status: `in progress; the backend split is landed, the OpenGL fallback is preserved, and Metal now reaches a first visible world-terrain milestone on Apple Silicon`
...
- The current Metal slice is intentionally narrow but now visibly useful:
  ...
  - world/terrain/mesh/minimap draw commands are still dropped on the Metal path while the playable-scene migration is pending
...
- `partial` The Metal backend now verifies cold launch, visible startup UI/menu, and first visible terrain; meshes, minimap, normal play, and editor support are still pending migration work.
...
- `partial` Port the runtime playable scene to Metal (terrain, meshes, minimap, scene presentation) now that startup UI/font rendering is live.
  ...
  - static meshes, skinned meshes, minimap, selection overlays, and normal gameplay flow are still pending
